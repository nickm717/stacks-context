...
### Achromatic chassis: brand accent removed, status palette collapsed to four hues
**Status:** Accepted · **Date:** 2026-08-16 · **Surface:** whole app, color tokens
**Decision.** The terracotta-clay accent is removed from the system entirely. Primary actions, selected chips, and active tabs are now a solid warm-near-black ink fill (`action/primary`, inverting to near-white in dark mode); links are underlined `text/primary` with no hue. Neutrals stay warm-cast (hue 72–78°) but drop from chroma 0.006 to **0.003**. The seven per-state lending colors collapse to four `status/*` hues — positive (green) / attention (amber) / overdue (red) / neutral (gray) — where hue answers "does this need me?" and the paired icon + label answer "which state exactly?" Two new roles are added: `background/cover-tint`, a per-book hue sampled from the cover with lightness and chroma clamped to fixed values, used on the **book detail header only**; and `feedback/scan-hit`, a cyan (~195°) reserved exclusively for scanner success and deliberately unused elsewhere.
**Why.** Two problems, one fix. First, terracotta (~42°) and `state-overdue` (~28°) were near-neighbors, and the book card is the one component guaranteed to render them adjacently — a terracotta "Request" button beside an overdue badge. Second, the accent was doing two incompatible jobs: brand signature *and* every CTA fill, which meant it appeared so often it signified nothing. Removing it solves both and hands the color budget to the covers, which is what a book app should be showing anyway. The four-hue collapse follows from a rule already in the system: color may never be the sole state signal, so with icon and label load-bearing, seven hues were paying for information they weren't allowed to carry. Reference for the whole direction is ElevenReader (Mobbin), which runs a fully achromatic shell and lets color appear only in content-derived surfaces and momentary feedback; Stripe Press and coffi confirm that warm-neutral + near-black ink stays warm without an accent.
**Rejected / considered.** *Keep terracotta, shift overdue to a cooler crimson (~15°)* — smaller change, but leaves the accent doing two jobs. *Keep all seven state hues* — rejected as paying token cost for redundant signal. *Cover tint on shelf/grid cards too* — rejected: twenty tinted cards is a polychrome browse screen, which defeats the achromatic chassis; detail header only. *True-neutral (chroma 0) grays, exactly like ElevenReader* — rejected as abandoning the "Corner Bookshelf" warmth; 0.003 keeps the cast readable beside a true-neutral UI without looking beige. *Raw sampled cover color, unclamped* — rejected: contrast against `text/primary` becomes unpredictable at design time; clamping lightness/chroma and keeping only hue makes it safe by construction.
**Affects.** Supersedes the color half of "Color, type, and spacing values locked" below (type and spacing from that entry stand). `DESIGN.md` §Colors rewritten. **Follow-up required:** the Figma `Color` variable collection and `tokens/colors.css` are both now out of sync and need a resync pass — and `tokens/colors.css` does not currently exist anywhere in this repo, so its location needs confirming. Also needs a pass: `design/component-spec.md` §3.C badge spec (seven state colors → four), any book-card or button specs referencing `action/primary` as terracotta, and the retired tokens list in `DESIGN.md`.

### Color, type, and spacing values locked; Figma variables library synced to code
**Status:** Accepted, **color values superseded** 2026-08-16 by "Achromatic chassis: brand accent removed, status palette collapsed to four hues" above · **Date:** 2026-08-16 · **Surface:** whole app, design tokens (Figma + code)
> The token *naming convention*, the no-primitives-tier decision, and all type/spacing/radius values in this entry still stand. The specific color values and the `text/on-accent` token name do not — see the superseding entry.

**Decision.** Claude Design generated the first full token package (`tokens/colors.css`, `typography.css`, `spacing.css`, plus React components) against the directions already locked in this log. That package was then rebuilt as a synced Figma variables library (file "Design System", `Color`/`Typography`/`Spacing`/`Radius` collections, Light + Dark modes) verified pixel-identical to the shipped CSS via OKLCH→sRGB spot-checks and WCAG AA contrast checks on every state fg/bg pair. Exact values are now recorded in `DESIGN.md`'s Colors and Typography sections rather than marked `[TODO]`. Design tokens are named `category/role/variant` on both sides — Figma variable `action/primary`, `status/overdue/bg`, `text/on-accent`; CSS custom property `--color-action-primary`, `--color-status-overdue-bg`, `--color-text-on-accent` — one Figma variable per CSS custom property, no build pipeline, kept in sync by hand re-verifying resolved hex values after any structural change.
**Why.** The color/type/spacing *directions* were settled 2026-08-10, but a design system needs exact values and a live source of truth designers can pull into Figma, not just tokens in a stylesheet. `category/role/variant` naming (rather than a flat list like the original `paper`/`ink`/`accent-tint`) makes the token's purpose legible from its name alone and matches how Figma's variable picker groups by `/`.
**Rejected / considered.** A primitives tier under semantic tokens (raw color ramp → semantic alias, e.g. `gray-500` → `background/default`) was fully prototyped in Figma — a 14-step neutral ramp, brand ramp, and per-state-hue ramps — then deliberately reverted. With ~36 semantic tokens and no case of one primitive value backing multiple semantic roles (most ramps only ever fed 1–2 tokens), the extra indirection added a second thing to keep in sync for no real payoff; it earns its cost only when a genuine multi-token fan-out shows up (e.g. swapping the brand hue in one place and having many tokens update). A three-tier DTCG-style architecture (primitives → semantic → component tokens) was also considered after reviewing an outside enterprise write-up, and rejected for the same reason plus the added cost of breaking 1:1 name parity with the already-shipped CSS custom properties.
**Gotcha found and fixed.** Figma variable *scopes* restrict which property pickers a variable shows up in. All `fg` (foreground/label) and `text/*` tokens were scoped to `TEXT_FILL` only, so they were invisible when picking a shape/icon fill — even though the code uses these same tokens for icon colors (SVG shapes), not just text. Fixed by widening those 15 tokens to `TEXT_FILL` + `SHAPE_FILL` + `STROKE_COLOR`. Worth checking scopes on any new color token that might apply to both text and iconography.
**Affects.** `DESIGN.md` (rewritten with locked values, SEED marker removed); Figma "Design System" file (`Foundations` page: two-frame Light/Dark color spec organized by intent with plain swatches, a type spec sheet, elevation and spacing/radius frames; `Components` page: `State Badge` variant set, `Book Card` master + demo frames); code design-system package (`tokens/*.css`, `components/**/*.jsx`) delivered as a zip. No visual/functional change to any resolved color — this was a documentation + naming + Figma-picker-visibility pass, not a value change.
...
**Status:** Accepted (values locked 2026-08-16, see "Color, type, and spacing values locked" below) · **Date:** 2026-08-10 · **Surface:** whole app, color tokens
...
**Affects.** `DESIGN.md` colors section — exact OKLCH values now locked, see below. Every semantic state badge inherits this direction.
